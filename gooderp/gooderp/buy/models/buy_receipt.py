    ...
    def _wrong_receipt_done(self):
        ...
        for line in self.line_in_ids:
            ...
            if line.goods_id.force_batch_one:
                #查询该设备是否有销售
                wh_move_sell_in_lines = self.env['wh.move.line'].search(
                    [('state', '=', 'done'), ('type', '=', 'in'), ('goods_id', '=', line.goods_id.id), ('lot','=',line.lot),('sell_line_id','!=',False )])
                wh_move_sell_out_lines = self.env['wh.move.line'].search(
                    [('state', '=', 'done'), ('type', '=', 'out'), ('goods_id', '=', line.goods_id.id), ('lot','=',line.lot),('sell_line_id','!=',False)])
                sell_in_total = sum(line.goods_qty for line in wh_move_sell_in_lines)
                sell_out_total = sum(line.goods_qty for line in wh_move_sell_out_lines)
                if sell_in_total < sell_out_total:
                    raise UserError(u'该产品已经销售给客户，不可以重复采购，请查询销售明细！\n商品:%s 序列号:%s' %
                                    (line.goods_id.name, line.lot))
                # 查询该设备是否有库存
                wh_move_in_lines = self.env['wh.move.line'].search(
                    [('state', '=', 'done'), ('type', '=', 'in'), ('goods_id', '=', line.goods_id.id), ('lot', '=', line.lot)])
                wh_move_out_lines = self.env['wh.move.line'].search(
                    [('state', '=', 'done'), ('type', '=', 'out'), ('goods_id', '=', line.goods_id.id), ('lot', '=', line.lot)])
                in_total = sum(line.goods_qty for line in wh_move_in_lines)
                out_total = sum(line.goods_qty for line in wh_move_out_lines)
                if in_total != out_total:
                    raise UserError(u'请检查仓库库存，仓库已存在库存的产品不允许重复采购！\n商品:%s 序列号:%s' %
                                    (line.goods_id.name, line.lot))

                    # for move_line in wh_move_in_lines:
    ...
    def buy_receipt_draft(self):
        ...
        for line in source_line:
            if line.money_id.state == 'done':
                ...
                #反审核采购入库、退货单，不能反审核付款的
                raise UserError(u'该单据存在已经审核的付款单，不允许反审核')
            # # 判断付款单 源单行 是否有别的行存在
            # other_source_line = []
        ...
        for invoice in invoice_ids:
            if invoice.reconciled != 0:
                raise UserError(u'单据已经结算不允许反审核')
