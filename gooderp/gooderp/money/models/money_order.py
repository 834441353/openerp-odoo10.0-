...
class MoneyOrder(models.Model):
    ...
    state = fields.Selection([
        ...
        ('done', u'已完成'),
        ('cancel', u'已作废'),
    ], string=u'状态', readonly=True, track_visibility='onchange',
...
class MoneyInvoice(models.Model):
    ...
    state = fields.Selection([
        ('draft', u'草稿'),
        ('done', u'完成')
    ], string=u'状态', track_visibility='onchange',
        default='draft', copy=False, index=True,
    ...
    def money_invoice_done(self):
        ...
        for inv in self:
            ...
            #if not inv.date_due:
            #    inv.date_due = fields.Date.context_today(self) + inv.partner_id.credit_day
            inv.date_due = datetime.datetime.strptime(fields.Date.context_today(self), '%Y-%m-%d') + datetime.timedelta(days=inv.partner_id.credit_day)
