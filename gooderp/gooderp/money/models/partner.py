...
class Partner(models.Model):
    ...
    def _set_receivable_init(self):
        ...
        money_invoice_id = self.env['money.invoice'].search([
            ('partner_id', '=', self.id),
            ('is_init', '=', True),('category_id','=',categ.id)])
        if money_invoice_id:
            if money_invoice_id.reconciled != 0:
                raise UserError(u'业务伙伴 %s 的期初金额已经被结算，不允许修改期初，请先反审核相应的结算单据' % (money_invoice_id.partner_id.name))
            else:
        ...
        if self.init_balance:
            ...
            other_money_init = self.with_context(type='other_get').env['other.money.order'].create({
                ...
                'line_ids': [(0, 0, {
                    ...
                    'price_taxed': self.init_balance,
                    'tax_rate': 0,
                    'tax_amount': 0,
                })],
                'receipt': self.init_balance,
                'state': 'draft',
