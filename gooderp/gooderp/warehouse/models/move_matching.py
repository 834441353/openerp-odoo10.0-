...
class WhMoveMatching(models.Model):
    ...
    move_in_search = fields.Char(string=u'入库单号',compute='_compute_move_search',store=True)
    move_out_search = fields.Char(string=u'出库单号',compute='_compute_move_search',store=True)

    @api.one
    @api.depends('line_in_id','line_out_id')
    def _compute_move_search(self):
        for move_line in self:
            if move_line.line_in_id:
                move_line.move_in_search = move_line.line_in_id.move_id.name
            if move_line.line_out_id:
                move_line.move_out_search = move_line.line_out_id.move_id.name
