...
class WhMove(models.Model):
    ...
    @api.one
    @api.depends('line_out_ids.goods_qty', 'line_in_ids.goods_qty')
    def _compute_total_qty(self):
        goods_total = 0
        if self.total_qty > 0:
            goods_total = self.total_qty
        if self.line_in_ids:
            # 入库商品总数
        ...
        _logger.debug(u'模型compute_total_qty：%s.出库移库ID%s.单号%s.入库移库ID%s.单据合计数量%s'%(self,self.line_out_ids ,self.name,self.line_in_ids,self.total_qty))
        _logger.debug(traceback.format_list(traceback.extract_stack(None,15)))
