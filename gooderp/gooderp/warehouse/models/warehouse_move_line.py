...
class WhMoveLine(models.Model):
    ...
    def compute_suggested_cost(self):
        ...
        if self.env.context.get('type') == 'in' and self.goods_id:
            # 销售退货取销售发货成本，否则如果商品上设置了税率，则按商品上的计算，否则按公司上的进项税率计算
            if self.sell_line_id:
                sell_delivery = self.env['sell.delivery'].search([('order_id', '=', self.sell_line_id.order_id.id),
                                                            ('is_return','=',False),('state','=','done')],limit=1)
