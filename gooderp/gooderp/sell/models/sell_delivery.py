...
class SellDelivery(models.Model):
    ...
    def _compute_all_amount(self):
        ...
        _logger.debug(u'模型compute_all_amount：发货单ID%s.单据合计%s.折扣%s.客户承担费用%s'%(self,self.amount, self.discount_amount, self.partner_cost))
        ...
        for line in self.line_in_ids:
            ...
            if line.goods_id.force_batch_one:
                ...
                wh_move_out_lines = self.env['wh.move.line'].search(
                    [('state', '=', 'done'), ('type', '=', 'out'), ('goods_id', '=', line.goods_id.id),
                     ('lot', '=', line.lot)])
        ...
        if self.order_id:
            for line in self.line_in_ids:
                if self.order_id.type == 'return':
                    ...
                    #判断销售退货是否属于后加的，如果不是，修改销售订单的已执行数量
    ...
    def sell_delivery_draft(self):
        ...
        for line in source_line:
            if line.money_id.state == 'done':
                ...
                raise UserError(u'该单据存在已经审核的收款单，不能反审核')
            # # 判断收款单 源单行 是否有别的行存在
            # other_source_line = []
        ...
        for invoice in invoice_ids:
            if invoice.reconciled != 0:
                raise UserError(u'发货单已收款或者核销，不能撤销发货！')
