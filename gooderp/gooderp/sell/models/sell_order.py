...
class SellOrder(models.Model):
    ...
    state = fields.Selection(SELL_ORDER_STATES, u'确认状态', readonly=True,
                             help=u"销货订单的确认状态", index=True, track_visibility='onchange',
    ...
    def sell_order_done(self):
        ...
        # 销售单 计算客户的 本次发货金额+客户应收余额 是否小于客户信用额度， 否则报错
        decimal_amount = self.env.ref('core.decimal_amount')
        if self.type== 'sell':
            if self.partner_id.credit_limit != 0:
                if float_compare(self.amount - self.pre_receipt + self.partner_id.receivable, self.partner_id.credit_limit,
                                 precision_digits=decimal_amount.digits) == 1:
                    raise UserError(u'销售金额 + 客户应收余额 - 本次收款金额 不能大于客户信用额度！\n\
                     单据金额:%s\n 客户应收余额:%s\n 本次收款金额:%s\n客户信用额度:%s' % (
                        self.amount, self.partner_id.receivable, self.pre_receipt, self.partner_id.credit_limit))
    ...
    def _generate_delivery(self, delivery_line):
        ...
        if self.type == 'sell':
            ...
            delivery_id.write({'line_in_ids': [
                (0, 0, line[0]) for line in delivery_line]})
        _logger.debug(u'模型generate_delivery：销售订单:%s.销售发货单:%s.业务伙伴:%s.源库位:%s.目的库:%s.业务员:%s.单据日期:%s.引用:%s.备注:%s.折扣率:%s.折扣额:%s.外币:%s.联系人:%s.'%(
            self.name,delivery_id.name,self.partner_id.name, warehouse.name,warehouse_dest.name,self.user_id.name,self.delivery_date,self.ref, self.note,self.discount_rate,self.discount_amount,self.currency_id.id, self.contact,))
        return delivery_id
