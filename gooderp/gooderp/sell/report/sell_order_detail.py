        ...
        cr.execute("""
            CREATE or REPLACE VIEW sell_order_detail AS (
                SELECT  MIN(wml.id) AS id,
                    ...
                    uom.name AS uom,
                    wml.price AS price,
                    wml.discount_amount AS discount_amount,
                    ...
                    SUM(CASE WHEN wm.origin = 'sell.delivery.sell' THEN wml.goods_qty
                        ELSE - wml.goods_qty END) * (wml.price - wml.cost_unit)
                        -(CASE WHEN wm.origin = 'sell.delivery.sell' THEN wml.discount_amount
                        ELSE - wml.discount_amount END) AS margin,
                ...
                GROUP BY wm.date, wm.name, origin, wm.user_id, wm.partner_id,
                    goods_code, goods.id, attribute, lot, wh.id, uom,
                    wml.price, wml.discount_amount, wml.cost_unit, sd.money_state, sd.return_state, wml.note,
                    mi.get_amount_date
                )
        """)
